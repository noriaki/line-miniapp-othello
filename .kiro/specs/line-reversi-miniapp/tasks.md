...
- [ ] 5.1 WASM モジュールのインタフェース検証
  - ai.wasm の全公開インタフェースの存在と型を検証
  - 必須関数 (\_init_ai, \_calc_value, \_malloc, \_free) の呼び出しテスト
  - メモリインタフェース (memory, HEAP8, HEAPU8) の動作確認
  - \_calc_value の追加パラメータ (a1?, a2?, a3?) の用途と影響を検証
  - _Requirements: 3.1, 4.1, 4.2, 4.3_

- [ ] 5.2 AI 計算精度のブラックボックステスト
  - 初期ボード、中盤、終盤の各状態で AI が合法手を返すことを検証
  - 返り値が 0-63 の範囲内であることを確認
  - AI が提案した手が実際に配置可能であることを検証
  - 複数回の計算で一貫性のある結果を返すことを確認
  - _Requirements: 3.2, 3.3, 4.4_

- [ ] 5.3 WASM パフォーマンステスト
  - AI 計算時間が 3秒以内であることを検証
  - 様々なボード状態での計算時間のばらつきを測定
  - メモリアロケーション/デアロケーションの正常動作を確認
  - 連続計算でのメモリリークがないことを検証 (10回連続)
